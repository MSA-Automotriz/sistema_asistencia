...
ticketsRouter.post('/tickets', validate(createTicketSchema), async (request, response) => {
...
ticketsRouter.patch('/tickets/:id/status', validate(updateTicketStatusSchema), async (request, response) => {
    const updated = await tickets.updateTicketStatus(request.auth, String(request.params.id), request.body);
