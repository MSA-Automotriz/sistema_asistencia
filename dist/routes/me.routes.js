...
meRouter.get('/me/profile', async (request, response) => ok(response, 'Perfil obtenido correctamente', await selfService.getProfile(request.auth.sub)));
...
meRouter.get('/me/last-attendance', async (request, response) => ok(response, 'Último registro de asistencia obtenido correctamente', await selfService.getLastAttendance(request.auth.sub)));
meRouter.get('/me/requests', async (request, response) => ok(response, 'Historial de solicitudes obtenido correctamente', await selfService.requestHistory(request.auth.sub)));
meRouter.get('/me/request-summary', async (request, response) => ok(response, 'Resumen de solicitudes obtenido correctamente', await selfService.requestStatusSummary(request.auth.sub)));
...
meRouter.get('/me/announcements', async (request, response) => ok(response, 'Anuncios obtenidos correctamente', await announcements.listForUser(request.auth.sub)));
meRouter.get('/me/devices', async (request, response) => ok(response, 'Dispositivos obtenidos correctamente', await selfService.listDevices(request.auth.sub)));
