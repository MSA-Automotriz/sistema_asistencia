...
companiesRouter.get('/companies/:companyId/settings', authorize('settings.read'), async (request, response) => {
...
companiesRouter.put('/companies/:companyId/settings', authorize('settings.update'), validate(companySettingsSchema), async (request, response) => {
    const companyId = String(request.params.companyId);
    await prisma.company.findUniqueOrThrow({ where: { id: companyId }, select: { id: true } });
    await prisma.$transaction(request.body.settings.map((setting) => prisma.setting.upsert({
...
companiesRouter.patch('/companies/:companyId/identity', authorize('companies.update'), validate(companyIdentitySchema), async (request, response) => {
