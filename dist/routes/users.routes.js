...
usersRouter.put('/users/:id/status', authorize('users.update'), validate(userStatusSchema), async (request, response) => {
...
usersRouter.post('/users/:id/reset-password', authorize('users.update'), validate(passwordSchema), async (request, response) => {
...
usersRouter.put('/users/:id/role', authorize('users.update'), validate(roleSchema), async (request, response) => ok(response, 'Rol asignado correctamente', await users.assignRole(String(request.params.id), request.body.roleId)));
usersRouter.put('/users/:id/permissions', authorize('users.update'), validate(permissionsSchema), async (request, response) => ok(response, 'Permisos del usuario actualizados correctamente', await users.replacePermissions(String(request.params.id), request.body.permissionIds)));
