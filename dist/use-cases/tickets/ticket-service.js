...
export class SupportTicketService {
    ...
    async createTicket(userId, input) {
        ...
        if (!companyId) {
            ...
            if (!defaultCompany)
                throw new AppError(400, 'No existe una empresa activa en el sistema');
    ...
    async listTickets(auth, filters) {
    ...
    async getTicketById(auth, ticketId) {
    ...
    async updateTicketStatus(auth, ticketId, input) {
        const isManager = await this.isManagerOrAdmin(auth);
        if (!isManager) {
        ...
        if (updated.userId !== auth.sub) {
            const statusLabel = updated.status === TicketStatus.RESOLVED
                ? 'RESUELTO'
            ...
            await prisma.notification.create({
                data: {
                    ...
                    body: updated.status === TicketStatus.RESOLVED && updated.resolutionNote
    ...
    async cancelTicket(auth, ticketId) {
        const ticket = await prisma.supportTicket.findUnique({ where: { id: ticketId } });
        if (!ticket) {
            throw new AppError(404, 'Ticket de soporte no encontrado');
        }
