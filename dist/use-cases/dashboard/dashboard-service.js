...
export class DashboardService {
    async summary() {
        ...
        return {
            ...
            indicadoresKpi: {
                ...
                horasPromedioPorEmpleado: this.toHours(metrics.present.length ? workedHours.totalMinutes / metrics.present.length : 0),
                permisosPendientes: pendingPermissions,
    ...
    async presentEmployees() {
        ...
        const checkIns = new Set(attendance.filter(({ type }) => type === 'CHECK_IN').map(({ employeeId }) => employeeId));
    ...
    dailyMetrics(employees, attendance) {
        const presentIds = new Set(attendance.filter(({ type }) => type === 'CHECK_IN').map(({ employeeId }) => employeeId));
        return {
    ...
    hoursWorked(attendance) {
        ...
        for (const event of attendance) {
            ...
            else if ((event.type === 'BREAK_OUT' || event.type === 'CHECK_OUT') &&
                accumulator.activeSegmentStart) {
                accumulator.workedMinutes += Math.max(0, (event.recordedAt.getTime() - accumulator.activeSegmentStart.getTime()) / 60_000);
                accumulator.activeSegmentStart = undefined;
            }
            perEmployee.set(event.employeeId, accumulator);
        }
        return {
            totalMinutes: [...perEmployee.values()].reduce((total, { workedMinutes }) => total + workedMinutes, 0)
        };
    }
    async attendanceTrend(todayStart, todayEnd) {
        ...
        for (const record of records) {
            ...
            if (record.type === 'CHECK_OUT')
                day.salidas += 1;
            if (record.type === 'CHECK_IN' && record.status === 'LATE')
                day.tardanzas += 1;
        }
        return [...trend.values()];
    }
