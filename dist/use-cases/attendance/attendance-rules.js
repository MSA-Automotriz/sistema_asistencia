...
export const parseWorkDaysConfig = (workDays) => {
    if (!workDays)
        return null;
...
export const resolveEffectiveDaySchedule = (schedule, type, recordedAt, timeZone) => {
...
export const getEffectiveBreakMinutesForToday = (schedule, recordedAt, timeZone) => {
    if (!schedule)
        return 0;
    const effective = resolveEffectiveDaySchedule(schedule, 'CHECK_IN', recordedAt, timeZone);
    return effective.breakMinutes;
};
export const determineAttendanceStatus = (type, schedule, recordedAt, timeZone, lastBreakOutAt) => {
    if (!schedule)
    ...
    if (type === 'BREAK_IN') {
        const allowedMinutes = effectiveSchedule.breakMinutes > 0
