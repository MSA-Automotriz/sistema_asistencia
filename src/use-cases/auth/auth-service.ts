...
export class AuthService {
  ...
  async setRecoveryQuestions(userId: string, questions: RecoveryQuestionInput[]) {
    ...
    if (normalizedQuestions.length < minimumRecoveryQuestions)
      throw new AppError(
        400,
        `Debe registrar al menos ${minimumRecoveryQuestions} preguntas de recuperación`
      );
    ...
    if (
      new Set(normalizedQuestions.map(({ question }) => question.toLocaleLowerCase())).size !==
      normalizedQuestions.length
    )
      throw new AppError(400, 'No puede repetir preguntas de recuperación');

    const records = await Promise.all(
      normalizedQuestions.map(async ({ question, answer }) => ({
        userId,
        question,
  ...
  async resetPasswordWithRecovery(
    ...
    const validAnswers =
      questions.length >= minimumRecoveryQuestions &&
  ...
  private async createSession(user: UserForSession, meta: RequestMeta, rememberMe: boolean) {
    ...
    const accessToken = jwt.sign(
      { sub: user.id, sid: session.id, role: user.role.name, permissions },
      env.JWT_SECRET,
