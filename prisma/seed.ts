...
const resources = ['users', 'roles', 'permissions', 'companies', 'sites', 'departments', 'positions', 'schedules', 'site-schedules', 'employees', 'attendances', 'vacations', 'work-permissions', 'overtime-requests', 'licenses', 'holidays', 'settings', 'notifications', 'announcements', 'devices', 'qr', 'reports', 'statistics', 'calendar', 'audit-logs', 'system-logs', 'backups', 'maintenance', 'imports', 'dashboard'];
...
async function main() {
  ...
  for (const roleName of roles.filter((name) => name !== 'Administrador')) await prisma.role.upsert({ where: { name: roleName }, update: {}, create: { name: roleName } });
